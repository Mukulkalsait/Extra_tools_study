...
# Info
  scrcpy --list-cameras = available cameras
  scrcpy --list-camera-sizes = sizes + (fast camera-fps options)
  scrcpy --list-display = display 
  scrcpy --list-apps = apps install
  scrcpy --list-encoders = encoders in phone
...
```bash 
  ...
  scrcpy 
    --video-source=camera / display 
    --camera-id=0 / display-id=0
    --render-driver=opengl 

    --camera-fps=60 
    --no-audio 
    --window-title "FaceCam" 
...
Usage: /etc/profiles/per-user/mukuldk/bin/scrcpy [options]
